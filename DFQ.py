'''
Data-Free Network QuantizationWith Adversarial Knowledge Distillation
'''
import argparse
...
from utils import Recorder, FeatureHook, MultiMeter, kdloss, get_loader


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--exp_name', type=str, default='DFQ-default')
    ...
    parser.add_argument('--weight_alpha', type=float, default=0.1)
...
if __name__ == '__main__':
    ...
    recorder = Recorder(base_path='result/main/DFQ',
    ...
    optimizer_s = optim.SGD(student.parameters(), lr=args.lr_s, momentum=0.9, weight_decay=5e-4, nesterov=True)

    scheduler_g = optim.lr_scheduler.CosineAnnealingLR(optimizer_g, args.epochs)
    scheduler_s = optim.lr_scheduler.CosineAnnealingLR(optimizer_s, args.epochs)

    _, test_loader = get_loader(args.dataset, 256, args.num_workers)
    test_num = len(test_loader.dataset)

    meter_train = MultiMeter()
    meter_train.register(['loss_s', 'loss_g', 'loss_psi'])
    meter_test = MultiMeter()
    meter_test.register(['loss', 'acc'])

    rescale = [5] + [1. for _ in range(len(hooks) - 1)]
    for epoch in range(args.epochs):
        ...
        for _ in tqdm(range(args.iters), leave=False, unit_scale=True, desc=f'Epoch-{epoch} train'):
            ...
            for i in range(5):
                ...
                # adversarial loss
                loss_s = args.weight_s * kdloss(s_logits, t_logits.detach())
            ...
            # adverserial loss
            loss_g = args.weight_g * -kdloss(s_logits, t_logits)

            # bns loss
            loss_bns = sum([mod.r_feature * rescale[idx] for (idx, mod) in enumerate(hooks)])

            # instance categorical entropy loss
            t_probs = F.softmax(t_logits, dim=1)
            loss_ice = (t_probs * torch.log10(t_probs)).sum(1).mean()

            # batch categorical entropy loss
            t_probs_mean = t_probs.mean(dim=0)
            loss_bce = -(t_probs_mean * torch.log10(t_probs_mean)).sum()

            # generator constraints
            loss_psi = args.weight_alpha * (loss_bns + loss_ice + loss_bce)
            # loss_psi = args.weight_alpha * (loss_bns)

            optimizer_g.zero_grad()
            # weighted loss
            (loss_g + loss_psi).backward()
            optimizer_g.step()

            meter_train.update('loss_g', loss_g.item())
            meter_train.update('loss_psi', loss_psi.item())
